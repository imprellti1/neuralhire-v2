...
export function getInterestLeadsRepositoryTests() {
  return [
    ...
    { name: 'interest-leads createLead e updateLead supabase mockado', run: async () => { __resetMemoryInterestLeadsForTests(); const previous = { publicInterest: env.PUBLIC_INTEREST_ACCOUNT_ID, interestLeads: env.INTEREST_LEADS_ACCOUNT_ID }; env.PUBLIC_INTEREST_ACCOUNT_ID = '7b8d9d4f-7c67-4a3f-8c85-5f6d5df1a114'; env.INTEREST_LEADS_ACCOUNT_ID = ''; try { const mock = createSupabaseMock(); __setInterestLeadsSupabaseClientForTests(mock, true); const created = await createInterestLead({ nome: 'Ana', empresa: 'Acme', email: 'ana@acme.com', status: 'novo' }); assertEqual(Boolean(created.id), true); assertEqual(created.account_id, '7b8d9d4f-7c67-4a3f-8c85-5f6d5df1a114'); const updated = await patchInterestLead(created.id, { status: 'contatado', observacoes: 'ligacao feita' }); assertEqual(updated.status, 'contatado'); } finally { env.PUBLIC_INTEREST_ACCOUNT_ID = previous.publicInterest; env.INTEREST_LEADS_ACCOUNT_ID = previous.interestLeads; } } },
    ...
    { name: 'interest-leads createEvent e listEvents supabase mockado', run: async () => { __resetMemoryInterestLeadsForTests(); const mock = createSupabaseMock(); __setInterestLeadsSupabaseClientForTests(mock, true); const created = await createInterestLead({ nome: 'Ana', empresa: 'Acme', email: 'ana@acme.com', status: 'novo' }); await createInterestLeadEvent(created.id, { tipo: 'observacao_adicionada', descricao: 'mensagem enviada' }); const events = await getInterestLeadEvents(created.id); assertEqual(Array.isArray(events), true); assertEqual(events.length > 0, true); } }
  ];
}
