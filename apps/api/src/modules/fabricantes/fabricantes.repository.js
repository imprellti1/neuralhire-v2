...
export async function createFabricante(data, options = {}) {
  const accountId = options.accountId || null;
  assertAccountId(accountId);
  const numericFields = ['pedido_minimo', 'boleto_minimo', 'comissao_padrao_percentual', 'prazo_maximo_dias'];
  ...
  const payload = {
    ...
    prazo_maximo_dias: data.prazo_maximo_dias === undefined || data.prazo_maximo_dias === null ? null : normalizeNumber(data.prazo_maximo_dias, 0),
...
export async function updateFabricante(id, data, options = {}) {
  ...
  const payload = {
    ...
    ...(data.prazo_maximo_dias !== undefined ? { prazo_maximo_dias: data.prazo_maximo_dias === null ? null : normalizeNumber(data.prazo_maximo_dias, 0) } : {}),
...
export async function createCondicaoPagamento(fabricanteId, data, options = {}) {
  ...
  const numericFields = ['parcelas', 'prazo_medio_dias', 'valor_minimo', 'percentual_acrescimo'];
  for (const field of numericFields) {
    if (data[field] !== undefined && data[field] !== null && Number(data[field]) < 0) {
      throw new BadRequestError('Valores invalidos', { domain: 'fabricantes' });
    }
...
export function __resetMemoryFabricantesForTests() { memoryFabricantes.length = 0; memoryCondicoes.length = 0; }
