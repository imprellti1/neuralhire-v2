...
export function registerVendedoresRoutes(router) {
  ...
  router.registerRoute({ method: 'POST', path: '/vendedores', domain: 'vendedores', middlewares: writeMiddlewares, schema: createVendedorSchema, handler: asyncHandler(async (req, res, context) => sendSuccess(res, await createVendedorHandler(context))) });
