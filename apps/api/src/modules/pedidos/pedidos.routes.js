...
export function registerPedidosRoutes(router) {
  ...
  router.registerRoute({ method: 'GET', path: '/pedidos/auditoria', domain: 'pedidos-comercial', schema: listPedidosAuditoriaQuerySchema, handler: asyncHandler(async (req, res, context) => sendSuccess(res, await getPedidosAuditoria(context))) });
