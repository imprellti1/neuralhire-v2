...
export async function getPedidosAuditoria(context = {}) {
  const accountId = getAccountIdFromContext(context);
  const query = context.query || {};
  const filters = {
...
export async function updatePedidoComissaoHandler(context = {}) {
  const accountId = getAccountIdFromContext(context);
  const body = { ...(context.body || {}) };
  const result = await updatePedidoComissao(context.params?.id, body, { accountId, context });
  return { ok: true, repositoryMode: getPedidosRepositoryMode(), ...result };
}

export async function updatePedidoFaturamentoHandler(context = {}) {
  const accountId = getAccountIdFromContext(context);
  const body = { ...(context.body || {}) };
