...
export function registerJobsRoutes(router) {
  ...
  router.registerRoute({ method: 'POST', path: '/jobs/diretor-reuniao-executiva/run', domain: 'system-jobs', handler: asyncHandler(async (req, res, context) => sendJson(res, 202, await runJobManualAdmin({ ...context, params: { ...(context.params || {}), id: 'diretor_reuniao_executiva' } }))) });
  router.registerRoute({ method: 'POST', path: '/jobs/diretor-delegacao/run', domain: 'system-jobs', handler: asyncHandler(async (req, res, context) => sendJson(res, 202, await runJobManualAdmin({ ...context, params: { ...(context.params || {}), id: 'diretor_delegacao' } }))) });
  router.registerRoute({ method: 'POST', path: '/jobs/whatsapp-learning/run', domain: 'system-jobs', handler: asyncHandler(async (req, res, context) => sendJson(res, 202, await runJobManualAdmin({ ...context, params: { ...(context.params || {}), id: 'whatsapp_learning_worker' } }))) });
  router.registerRoute({ method: 'POST', path: '/jobs/whatsapp-learning-cognitive/run', domain: 'system-jobs', handler: asyncHandler(async (req, res, context) => sendJson(res, 202, await runJobManualAdmin({ ...context, params: { ...(context.params || {}), id: 'whatsapp_learning_cognitive_worker' } }))) });
