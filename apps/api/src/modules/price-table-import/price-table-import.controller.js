...
import { executePriceTableImport, previewPriceTableImport } from './price-table-import.repository.js';

function assertContextAccount(context = {}) {
  const accountId = getAccountIdFromContext(context);
  if (!accountId) throw new ForbiddenError('Contexto de tenant obrigatorio', { code: 'TENANT_REQUIRED', domain: 'price-table-import' });
  return accountId;
}
...
export async function previewPriceTableImportHandler(context = {}) {
  const accountId = assertContextAccount(context);
  const file = resolveImportFile(context);
  const buffer = parseBase64File(file);
  if (!buffer) throw new BadRequestError('Arquivo XLSX obrigatorio', { domain: 'price-table-import' });
  return previewPriceTableImport({ accountId, fileName: file.fileName || file.filename || 'tabela-preco.xlsx', buffer });
}

export async function executePriceTableImportHandler(context = {}) {
  const accountId = assertContextAccount(context);
  const importToken = resolveImportToken(context);
  if (!importToken) throw new BadRequestError('importToken obrigatorio', { domain: 'price-table-import' });
  return executePriceTableImport({ accountId, importToken });
}
