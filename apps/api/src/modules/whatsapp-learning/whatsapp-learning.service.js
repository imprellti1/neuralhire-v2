...
import { buildWhatsappLearningNormalizedPayload, listNormalizedLearningEvents, listPendingLearningEvents, normalizeLearningEvent } from './whatsapp-learning.repository.js';
...
export async function runWhatsappLearningWorker(context = {}) {
  ...
  for (const event of normalizedEvents) {
    try {
      ...
      const analysis = await analyzeLearningEvent({
        ...
        normalized_payload: event.normalized_payload && typeof event.normalized_payload === 'object' ? event.normalized_payload : {},
      ...
      await normalizeLearningEvent(event.id, {
        ...
        normalized_payload: {
          ...(event.normalized_payload && typeof event.normalized_payload === 'object' ? event.normalized_payload : {}),
          cognitive: {
            status: analysis.status || 'disabled',
            provider: analysis.provider ?? null,
            model: analysis.model ?? null,
  ...
  return { ok: true, processed, cognitivelyProcessed, failed, scanned: events.length + normalizedEvents.length };
}
