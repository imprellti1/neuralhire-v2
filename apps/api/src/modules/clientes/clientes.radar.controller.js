...
import { getClientesRadar, recalcularRadarComercial } from './clientes.radar.service.js';
...
export async function recalcularClientesRadarHandler(context = {}) {
  const accountId = getAccountIdFromContext(context);
  const query = context.query || {};
  const filtros = {
    vendedor_id: parseFilter(query.vendedor_id),
  ...
  const result = await recalcularRadarComercial({ context, accountId, filtros, supabase: context.supabase });
