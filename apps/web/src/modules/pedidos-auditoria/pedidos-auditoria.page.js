...
import { fetchPedidosAuditoria, patchPedidoComissao, patchPedidoFaturamento } from './pedidos-auditoria.service.js';
    ...
    return `
      <div class="nha2-overlay" data-testid="nha2-modal-overlay">
        <div class="nha2-modal">
          <h3>${isComissao ? 'Definir comissão' : 'Marcar faturado'}</h3>
          ...
          <div class="nha2-modal-actions">
            ...
            <button class="nha2-btn nha2-small" id="nha2-save">${isComissao ? 'Salvar comissão' : 'Salvar faturamento'}</button>
          </div>
        </div>
      </div>`;
  }
    ...
    root.innerHTML = `
      <section class="nha2-head">
        <div>
          ...
          <div class="nha2-sub">Lista operacional para corrigir comissão, visualizar pedidos sem itens e marcar faturamento total.</div>
        </div>
      ...
      <section class="nha2-panel">
        ${state.loading ? '<div class="nha2-empty">Carregando pedidos...</div>' : state.error ? '<div class="nha2-error">Não foi possível carregar a auditoria.</div>' : !state.items.length ? '<div class="nha2-empty">Nenhum pedido com problema encontrado.</div>' : `
          ...
          <table class="nha2-table">
            <thead><tr><th>Número ERP</th><th>Cliente</th><th>Status</th><th>Emissão</th><th>Faturamento</th><th>Total</th><th>Comissão Principal %</th><th>Comissão Preposto %</th><th>Itens</th><th>Problemas</th><th>Ações</th></tr></thead>
            <tbody>
              ${state.items.map((item) => `
                <tr class="nha2-row">
                  ...
                  <td>${(item.issues || []).map((issue) => `<span class="nha2-badge ${issue === 'sem_itens' ? 'warn' : issue === 'sem_comissao' ? 'danger' : 'ok'}">${issueLabel(issue)}</span>`).join('')}</td>
                  <td>
