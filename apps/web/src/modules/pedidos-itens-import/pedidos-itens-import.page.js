...
function renderRows(rows = []) {
  ...
  return `<table class="npi3-table" data-testid="preview-table"><colgroup><col style="width:96px"><col style="width:auto"><col style="width:130px"><col style="width:88px"><col style="width:88px"><col style="width:150px"></colgroup><thead><tr><th>Código ERP</th><th>Produto</th><th>Cor</th><th>Tamanho</th><th class="npi3-col-qty">Quantidade</th><th class="npi3-col-status">Status de Vínculo</th></tr></thead><tbody>${rows.map((row) => {
    ...
    return `<tr><td>${readText(row.codigo_produto_erp_original ?? row.codigo_erp ?? row.codigoERP ?? row.codigo)}</td><td><span class="npi3-product" title="${readText(row.nome_produto_original ?? row.produto)}">${readText(row.nome_produto_original ?? row.produto)}</span></td><td>${readText(row.cor_original ?? row.cor)}</td><td>${readText(row.tamanho_original ?? row.tamanho)}</td><td class="npi3-col-qty">${readText(row.quantidade)}</td><td class="npi3-col-status"><span class="npi3-chip ${badgeClass(status)}">${friendlyStatus(status)}</span></td></tr><tr class="npi3-reason-row"><td colspan="6"><span class="npi3-reason ${reasonClass}"><span class="npi3-reason-icon">${reasonIcon}</span> <span class="npi3-reason-text">${status === 'vinculado' ? 'Produto vinculado com sucesso' : reason}</span></span></td></tr>`;
  }).join('')}</tbody></table>`;
}
