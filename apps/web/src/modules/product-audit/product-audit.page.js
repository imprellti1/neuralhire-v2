...
export function renderProductAuditPage(root, { apiClient } = {}) {
  ...
  function render() {
    ...
    const hasPages = (pagination.totalPages || 1) > 1;
    const pager = hasPages ? `<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-top:12px"><div class="nha-state" style="padding:0;text-align:left">Mostrando ${(pagination.page || 1)} de ${(pagination.totalPages || 1)} páginas, ${(pagination.total || 0)} produtos com problema</div><div style="display:flex;gap:8px"><button class="nha-btn" id="nha-prev" ${pagination.page <= 1 ? 'disabled' : ''}>Anterior</button><button class="nha-btn" id="nha-next" ${pagination.page >= pagination.totalPages ? 'disabled' : ''}>Próxima</button></div></div>` : '';
