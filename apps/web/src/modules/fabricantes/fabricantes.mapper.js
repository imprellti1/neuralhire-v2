...
export function mapFabricantesData(response = {}) {
  ...
  return {
    items: items.map((item) => ({
      ...
      valorMinimoDuplicataExibicao: Number((item?.valor_minimo_duplicata ?? item?.pedido_minimo ?? item?.pedido_minimo_valor) || 0),
      pedidoMinimoExibicao: Number((item?.pedido_minimo_valor ?? item?.pedido_minimo ?? item?.valor_minimo_duplicata) || 0),
