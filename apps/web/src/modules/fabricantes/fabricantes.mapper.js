...
export function mapFabricantesData(response = {}) {
  ...
  return {
    items: items.map((item) => ({
      ...
      valorMinimoDuplicataExibicao: Number((item?.valor_minimo_duplicata ?? item?.pedido_minimo_valor ?? item?.pedido_minimo) || 0),
      pedidoMinimoExibicao: Number((item?.valor_minimo_duplicata ?? item?.pedido_minimo_valor ?? item?.pedido_minimo) || 0),
