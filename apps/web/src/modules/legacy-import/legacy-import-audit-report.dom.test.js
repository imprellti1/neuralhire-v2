...
test('legacy import audit report renders audit card and action', async () => {
  ...
  const apiClient = {
    get: async (path) => {
      ...
      if (path === '/legacy-import/batches/batch-1/records') return { records: [] };
      if (path === '/legacy-import/batches/batch-1/issues') return { issues: [] };
      if (path === '/legacy-import/batches/batch-1/audit') return { report: { batchId: 'batch-1', summary: { created: { clientes: 1 }, updated: {}, skipped: {}, failed: {} }, integrity: { orphanOrders: 0, orphanItems: 0, missingCustomers: 0, missingProducts: 0, missingVendors: 0, missingManufacturers: 0 }, warnings: [], errors: [] } };
  ...
  await flush();
  assert.match(document.body.textContent, /Executar Auditoria/);
  assert.match(document.body.textContent, /Clientes Importados/);
